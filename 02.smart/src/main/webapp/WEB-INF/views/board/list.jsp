...
<div class="col-xl-3-20 col-md-6 mb-3">
    <div class="card">
        <div class="card-body">
...
<tr>
	<td colspan="5">방명록글이 없습니다.</td>
</tr>
</c:if>
<c:forEach items="${page.list}" var="vo">
<tr><td>${vo.no}</td>
	<td class="text-start text-truncate"><a class="text-link" href="javascript:info(${vo.id})">${vo.title}</a>
		<c:if test="${vo.filecnt gt 0}"><i class="fa-solid fa-paperclip"></i></c:if></td>
	<td>${vo.name}</td>
	<td>${vo.writedate}</td>
	<td>${vo.readcnt}</td>
</tr>

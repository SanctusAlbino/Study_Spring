...
<!-- 댓글이 없는 경우 -->
	...
	<c:choose>
		<c:when test="${empty vo.profile}">
			<c:set var="profile" value="<i class='font-profile fa-regular fa-circle-user'></i>"></c:set>
		</c:when>
		<c:otherwise>
	...
	<div class="col-md-10 content border-bottom py-3" data-id="${vo.id}">
		<div class="d-flex align_items-center mb-2 justify-content-between">
	
			<div class="d-flex align_items-center mb-2">
...
function modifyStatus(_content){
	...
	//.comment 안에있던 댓글 내용은 textarea에 보여지게
...
function stayStatus(_content){
	...
	_content.find('.title').addClass('d-none'); //글자수부분 보이게
